...
valueses = { 100, 150, 200, 250, 300, 350, 400, 450, 500 }
...
function Lobby:__init( config )
	...
	vmoney = valueses[math.random(#valueses)]
	...
	if state == GamemodeState.WAITING then
		...
		elseif self.startingTime == 0 and self.timer:GetSeconds() > 600 then
			...
			if playerCount == 0 then
				Tron.Broadcast( "Трон в режиме ожидания, введите /tron, чтобы присоединиться к очереди. Главный приз: $" .. vmoney .. "! (Карта: " .. self.name .. ")", Color( 228, 142, 56 ) )
			else
				Tron.Broadcast( "Трон с " .. playerCount .. " игрок" .. (playerCount == 1 and "ом" or "ами") .. " в ожидание начала! (Карта: " .. self.name .. ")", Color( 228, 142, 56 ) )
			end
...
function Lobby:Disband( winner )
	...
	if winner then
		...
		Chat:Broadcast( "[Трон] ", Color.White, winner:GetName(), winner:GetColor(), " победил в трон и выйграл $" .. vmoney .. "!", Color( 228, 142, 56 ) )
	end
