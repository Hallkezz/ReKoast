...
function Tron:Render( args )
	...
	if self.state == GamemodeState.WAITING then
		...
		if playersNeeded > 0 then
			DrawCenteredShadowedText( Vector2( Render.Width / 2, 70 + TextSize.Large ), '(' .. 'Нужно на ' .. playersNeeded .. ' больше' .. (playersNeeded ~= 1 and 'игроков)' or ' игроков)'), Color( 165, 165, 165 ), 25 )
		end
	...
	elseif self.state == GamemodeState.INPROGRESS then
		...
		if LocalPlayer:InVehicle() then
			if gamepad then
				DrawCenteredShadowedText( Vector2( Render.Width / 2, Render.Height - 35 ), "Нажмите RB, чтобы заморозить свой след.", Color.Yellow )
			else
				DrawCenteredShadowedText( Vector2( Render.Width / 2, Render.Height - 35 ), "Нажмите правой кнопкой мыши, чтобы заморозить свой след.", Color.Yellow )
			end
		else
			if gamepad then
				DrawCenteredShadowedText( Vector2( Render.Width / 2, Render.Height - 35 ), "Вы проиграли. Нажмите LB или RB, чтобы начать слежку за другим игроком.", Color.Yellow )
			else
				DrawCenteredShadowedText( Vector2( Render.Width / 2, Render.Height - 35 ), "Вы проиграли. Нажмите левой или правой кнопкой мыши, чтобы начать слежку за другим игроком.", Color.Yellow )
			end
		end
