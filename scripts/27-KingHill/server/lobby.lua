	...
	if state == GamemodeState.WAITING then
		...
		elseif self.startingTime == 0 and self.timer:GetSeconds() > 900 then
			...
			if playerCount == 0 then
				KingHill.Broadcast( "Царь Горы в режиме ожидания, введите /khill, чтобы присоединиться к очереди. Главный приз: $" .. vmoney .. "! (Карта: " .. self.name .. ")", Color( 228, 142, 56 ) )
			else
...
function Lobby:Disband( winner )
	...
	if winner then
		...
		Chat:Broadcast( "[Царь Горы] ", Color.White, winner:GetName(), winner:GetColor(), " забрался на вершину первым и выйграл $" .. vmoney .. "!", Color( 228, 142, 56 ) )
