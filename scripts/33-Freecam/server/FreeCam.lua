...
function FreeCamManager:StoreTrajectory(args, client)
	if not FreeCamManager.CheckWhiteList(client) then
		client:SendChatMessage(string.format("%s Ошибка: Нет разрешения на сохранение траекторий!",
	...
	if args.type == nil or name == nil then
		client:SendChatMessage(string.format("%s Использование: /freecam <save/load/delete> <trajectory_name>",
								Config.name), Config.color)
		return
	end
	if args.type == "save" then
		if args.trajectory == nil and args.position == nil then
			client:SendChatMessage(string.format("%s Траектория не найдена!", Config.name), Config.colorError)
			return
		end
		local fileName = nil
		local typeName = nil
		...
		for line in io.lines(fileName) do
			...
			if exists or exists2 then
				client:SendChatMessage(string.format("%s %s с этим именем уже существует!",
		...
		if args.position then
			...
			client:SendChatMessage(string.format("%s Сохраненная позиция '%s'!",
			...
			file:write("\n")
			client:SendChatMessage(string.format("%s Сохраненная траектория '%s' с %d путевыми точками!",
	...
	elseif args.type == "load" then		
		...
		if not found then
			client:SendChatMessage(string.format("%s Ошибка: Траектория '%s' не найдена!",
	...
	elseif args.type == "delete" then		
		...
		if found then			
			client:SendChatMessage(string.format("%s Удаленная траектория '%s'",
