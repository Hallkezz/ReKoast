...
function CameraView:__init()
	self.enabled = true
	self.camera = 3
	self.selectedViews = { [0] = true, [1] = true, [2] = true, [3] = true }
	self.antiSnap = false
	...
	--				Hood Cam			Cockpit Cam			Chase Cam
	-- Land Vehicles
...
function CameraView:CalcView()
	...
	if Game:GetSetting( GameSetting.GamepadInUse ) == 0 then
		if Key:IsDown(4) then
			if self.camera == 3 then -- MMB resets zoom level in standard camera
				if self.enabled then
					self.zoom = 0
				end
			elseif self.reverse == false then
				self.reverse = true
				Events:Fire( "CameraState",
				{
					enabled = self.enabled,
					camera = self.names[self.camera],
					altview = self.reverse
				} )
			end
		elseif self.reverse == true and self.camera ~= 4 then
			self.reverse = false
			Events:Fire( "CameraState",
			{
				enabled = self.enabled,
	...
	if self.reverse == true and ( self.camera ~= 4 ) then
	...
	elseif self.camera == 0 then -- Bumper/Hood Cam
		local pos_3d = vehicle:GetPosition()
		t = Vector3( (self.vehicles[vid][3]), (self.vehicles[vid][1]), (self.vehicles[vid][2]) )
		cam_pos = pos_3d + ( angle * t )
	elseif self.camera == 1 then -- Chase Cam
		local pos_3d = vehicle:GetPosition()
		t = Vector3( (self.vehicles[vid][9]), (self.vehicles[vid][7]), (self.vehicles[vid][8]) )
		cam_pos = pos_3d + ( angle * t )
		angle = angle * Angle(0,-0.10,0)
	elseif self.camera == 2 then -- Bird's Eye View
		...
		angle.pitch = math.pi*1.51, 0
		angle.yaw = 0
	elseif self.camera == 3 then -- Standard Camera
...
function CameraView:MouseScroll( args )
	...
	if self.enabled then
		if self.zoom <= 20 then
			self.zoom = self.zoom + ( args.delta * 0.25 )
		else
			self.zoom = 20
		end
	end
...
function CameraView:LocalPlayerInput( args )
	if Game:GetState() ~= GUIState.Game then return end
	if LocalPlayer:InVehicle() and self.enabled then
		if args.input == Action.VehicleCam then return false end

		if args.input == Action.ActivateExtractionBeacon and Game:GetSetting( GameSetting.GamepadInUse ) == 1 then
			self.zoom = self.zoom + 0.05
			return false
		end

		if args.input == Action.EquipBlackMarketBeacon and Game:GetSetting( GameSetting.GamepadInUse ) == 1 then
			self.zoom = self.zoom - 0.05
			return false
		end
...
function CameraView:KeyUp( args )
	if LocalPlayer:InVehicle() and self.enabled then
		if args.key == string.byte("C") then
			if ( self.timer:GetMilliseconds() - self.lastTime ) > 50 then
				self:CycleViews()
			...
			self.lastTime = self.timer:GetMilliseconds()
		end
	end
end
