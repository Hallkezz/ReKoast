...
function Derby:__init(name, manager, world)
	...
	self:MessageGlobal( "Дерби в режиме ожидания, введите /derby, чтобы присоединиться к очереди. (Карта: " .. self.spawns.Location .. ")" )
end
...
function Derby:ModuleUnload()
	for k,p in pairs(self.eventPlayers) do
		if (self.state ~= "Lobby") then
			p:Leave()
			self:MessagePlayer(p.player, "Дерби отключен. Вы вернулись в исходное положение.")
